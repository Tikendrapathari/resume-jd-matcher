...
# Updated CORS configuration - Allow all origins for testing
CORS(app, resources={
    r"/api/*": {
        "origins": "*",
...
def extract_skills(text):
    ...
    skill_keywords = [
        ...
        'html', 'css', 'nodejs', 'express', 'angular', 'vue', 'c++', 'csharp',
        'php', 'laravel', 'wordpress', 'photoshop', 'illustrator', 'figma'
    ]
...
def match_resume():
    ...
    try:
        ...
        print(f"📁 JD file: {jd_file.filename}")
        ...
        if not allowed_file(resume_file.filename):
            return jsonify({'error': f'Invalid resume file type. Allowed: pdf, docx, txt'}), 400
        if not allowed_file(jd_file.filename):
            return jsonify({'error': f'Invalid job description file type. Allowed: pdf, docx, txt'}), 400
        ...
        print(f"💾 Files saved temporarily")
        ...
        if not resume_text or not jd_text:
            return jsonify({'error': 'Could not extract text from files. Please check file format and content.'}), 400
        
        # Calculate match score
        result = calculate_match_score(resume_text, jd_text)
        
        print(f"✅ Match score: {result['total_match']}%")
...
def test():
    """Test endpoint"""
    return jsonify({'message': 'Backend is working perfectly!'})
